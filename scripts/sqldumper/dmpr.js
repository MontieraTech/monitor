...
var getReportFilename = function (report) {
	...
	getSQLFields = function (report) {
		...
		if (report.aggregate) {
			...
			sqlFields = sqlFields.concat(report.aggregate.map(function (aggColumnSettings) {

				return aggColumnSettings.dbcolumn;

			}));
		}
		sqlFields = sqlFields.map(function (columnName) { return util.format("`%s`", columnName); }).join(", ");
